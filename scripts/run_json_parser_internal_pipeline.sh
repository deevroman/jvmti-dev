...
OUTPUT_DIR="$ROOT_DIR/.artifacts/pipeline_json_parser_internal"
LLM_ONLY=0
SOCKET_MODE=0
SOCKET_HOST=""
SOCKET_PORT=""
SOCKET_TIMEOUT=""
PIPELINE_NAMES=(
  "json_parser_parse_string"
  "json_parser_parse_reader"
  "json_parser_parse_json_reader"
  "streams_parse"
)
PIPELINE_CONFIGS=(
  "$ROOT_DIR/config_json_parser_internal_parse_string.json"
  "$ROOT_DIR/config_json_parser_internal_parse_reader.json"
  "$ROOT_DIR/config_json_parser_internal_parse_json_reader.json"
  "$ROOT_DIR/config_json_parser_internal_streams_parse.json"
)
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      ;;
    *)
      if [[ "$OUTPUT_DIR" == "$ROOT_DIR/.artifacts/pipeline_json_parser_internal" ]]; then
        OUTPUT_DIR="$1"
      else
...
for index in "${!PIPELINE_NAMES[@]}"; do
  ...
  echo "[$((index + 1))/${#PIPELINE_NAMES[@]}] Running internal JSON parser pipeline: $pipeline_name"
  "${CMD[@]}"
done
