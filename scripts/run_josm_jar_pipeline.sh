...
OUTPUT_DIR="$ROOT_DIR/.artifacts/pipeline_josm_program_arguments"
AGENT_LIB="${AGENT_LIB:-}"
JOSM_PREF="${JOSM_PREF:-/tmp/josm/settings}"
JOSM_HOME="${JOSM_HOME:-/tmp/josm}"
JOSM_ARGS=("${JOSM_ARGS[@]:---debug}")
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --*)
      ...
      ;;
    *)
      if [[ "$OUTPUT_DIR" != "$ROOT_DIR/.artifacts/pipeline_josm_program_arguments" ]]; then
...
echo "[1/2] Running josm.jar with JVMTI agent"
...
java \
  ...
  -cp "$JAR_FILE" "$APP_MAIN" "${JOSM_ARGS[@]}" > "$APP_LOG" 2>&1 &
APP_PID=$!
